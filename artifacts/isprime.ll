; ModuleID = 'isprime'
source_filename = "isprime.pas"

@__minipas_argc = internal global i32 0
@__minipas_argv = internal global ptr null
@number = internal global i64 0
@0 = private unnamed_addr constant [5 x i8] c"true\00", align 1
@1 = private unnamed_addr constant [6 x i8] c"false\00", align 1
@2 = private unnamed_addr constant [3 x i8] c"%s\00", align 1
@3 = private unnamed_addr constant [2 x i8] c"\0A\00", align 1

declare i32 @printf(ptr, ...)

declare i32 @atoi(ptr)

define i1 @prime(i64 %value) {
entry:
  %result = alloca i1, align 1
  %divisor = alloca i64, align 8
  %value1 = alloca i64, align 8
  store i64 %value, ptr %value1, align 4
  store i64 0, ptr %divisor, align 4
  store i1 false, ptr %result, align 1
  %value.value = load i64, ptr %value1, align 4
  %less.value = icmp slt i64 %value.value, 2
  br i1 %less.value, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  store i1 false, ptr %result, align 1
  br label %if.end

if.end:                                           ; preds = %while.end, %if.then
  %result.value9 = load i1, ptr %result, align 1
  ret i1 %result.value9

if.else:                                          ; preds = %entry
  store i64 2, ptr %divisor, align 4
  store i1 true, ptr %result, align 1
  br label %while.cond

while.cond:                                       ; preds = %if.end7, %if.else
  %divisor.value = load i64, ptr %divisor, align 4
  %divisor.value2 = load i64, ptr %divisor, align 4
  %mul.value = mul i64 %divisor.value, %divisor.value2
  %value.value3 = load i64, ptr %value1, align 4
  %lesseq.value = icmp sle i64 %mul.value, %value.value3
  %result.value = load i1, ptr %result, align 1
  %and.value = and i1 %lesseq.value, %result.value
  br i1 %and.value, label %while.body, label %while.end

while.body:                                       ; preds = %while.cond
  %value.value4 = load i64, ptr %value1, align 4
  %divisor.value5 = load i64, ptr %divisor, align 4
  %mod.value = srem i64 %value.value4, %divisor.value5
  %equal.value = icmp eq i64 %mod.value, 0
  br i1 %equal.value, label %if.then6, label %if.end7

while.end:                                        ; preds = %while.cond
  br label %if.end

if.then6:                                         ; preds = %while.body
  store i1 false, ptr %result, align 1
  br label %if.end7

if.end7:                                          ; preds = %if.then6, %while.body
  %divisor.value8 = load i64, ptr %divisor, align 4
  %add.value = add i64 %divisor.value8, 1
  store i64 %add.value, ptr %divisor, align 4
  br label %while.cond
}

define i32 @main(i32 %argc, ptr %argv) {
entry:
  store i32 %argc, ptr @__minipas_argc, align 4
  store ptr %argv, ptr @__minipas_argv, align 8
  %argv.value = load ptr, ptr @__minipas_argv, align 8
  %argv.slot = getelementptr inbounds ptr, ptr %argv.value, i64 1
  %argv.text = load ptr, ptr %argv.slot, align 8
  %arg.number = call i32 @atoi(ptr %argv.text)
  %arg.integer = sext i32 %arg.number to i64
  store i64 %arg.integer, ptr @number, align 4
  %number.value = load i64, ptr @number, align 4
  %prime.result = call i1 @prime(i64 %number.value)
  %0 = select i1 %prime.result, ptr @0, ptr @1
  %1 = call i32 (ptr, ...) @printf(ptr @2, ptr %0)
  %2 = call i32 (ptr, ...) @printf(ptr @3)
  ret i32 0
}
